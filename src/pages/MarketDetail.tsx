...
import { ASKS, BIDS, CANDLES } from '../data/mock'
...
const STATS = [
  { label: '24h Volume',    value: '$48.2M' },
  { label: 'Market Cap',    value: '$1.22B' },
  { label: 'Open Interest', value: '$12.6M' },
  { label: 'Funding',       value: '+0.0125%', green: true },
  { label: 'Holders',       value: '3,184' },
  { label: 'Supply',        value: '6,742' },
  { label: 'All-Time High', value: '2,688.00' },
]
...
function MiniChart() {
  ...
  useEffect(() => {
    ...
    const toX   = (i: number) => pad.left + i * (cW / candles.length) + (cW / candles.length) / 2
...
export default function MarketDetail() {
  ...
  const maxAsk = Math.max(...ASKS.map(l => l.size))
  const maxBid = Math.max(...BIDS.map(l => l.size))
  const maxSz  = Math.max(maxAsk, maxBid)
    ...
    <div style={{ flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ height: 96, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', borderBottom: '1px solid var(--border-subtle)', background: 'var(--surface-1)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 18 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, padding: '2px 7px', borderRadius: 4, background: 'rgba(160,81,252,0.12)', color: '#A051FC' }}>PERP · 20x</span>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 22 }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 3 }}>
            <span style={{ fontSize: 28, fontWeight: 800, fontFamily: 'var(--font-mono)', fontVariantNumeric: 'tabular-nums', color: '#fff' }}>2,456.00</span>
            <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--up-500)' }}>+121.40 · +5.21%</span>
          </div>
          <div style={{ display: 'flex', gap: 10 }}>
            <button onClick={() => navigate('/trade')} style={{ height: 46, padding: '0 20px', borderRadius: 6, fontSize: 14, fontWeight: 900, background: 'var(--buy)', color: '#fff', border: 'none', cursor: 'pointer' }}>Buy / Long</button>
            <button onClick={() => navigate('/trade')} style={{ height: 46, padding: '0 20px', borderRadius: 6, fontSize: 14, fontWeight: 900, background: 'var(--sell)', color: '#fff', border: 'none', cursor: 'pointer' }}>Sell / Short</button>
          </div>
        </div>
      </div>
      ...
      <div style={{ height: 72, flexShrink: 0, display: 'flex', alignItems: 'center', padding: '0 24px', borderBottom: '1px solid var(--border-subtle)', background: 'var(--bg-base)' }}>
        {STATS.map((s, i) => (
          <div
            key={s.label}
            style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 4, padding: i === 0 ? '0 24px 0 0' : '0 24px', borderLeft: i > 0 ? '1px solid var(--border-subtle)' : 'none' }}
          >
            <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-tertiary)' }}>{s.label}</span>
            <span style={{ fontSize: 16, fontWeight: 700, fontFamily: 'var(--font-mono)', fontVariantNumeric: 'tabular-nums', color: s.green ? 'var(--up-500)' : 'var(--text-primary)' }}>
              {s.value}
            </span>
          </div>
        ))}
          ...
          <div style={{ flex: 1, minHeight: 0, background: 'var(--surface-1)', border: '1px solid var(--border-subtle)', borderRadius: 10, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
            <div style={{ height: 44, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px', borderBottom: '1px solid var(--border-subtle)' }}>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
                ...
                <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>USDC per NFT · last 24h</span>
              </div>
            ...
            <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, padding: '8px 16px 0' }}>
              <span style={{ fontSize: 22, fontWeight: 800, fontFamily: 'var(--font-mono)', color: 'var(--text-primary)' }}>{formatPrice(last.close)}</span>
              <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--up-500)' }}>+5.21%</span>
          ...
          <div style={{ flexShrink: 0, background: 'var(--surface-1)', border: '1px solid var(--border-subtle)', borderRadius: 10, padding: '20px 22px', display: 'flex', gap: 36 }}>
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <p style={{ margin: 0, fontSize: 13, lineHeight: 1.6, color: 'var(--text-secondary)' }}>
                LNYQ Genesis is a 6,742-piece pixel collection made fungible by the LNYQ order book. Rather than listing one-off auctions, every NFT trades against a deep central limit order book — spot for instant settlement, perps for leveraged exposure. Floor price discovery is continuous and on-chain.
              </p>
            </div>
            <div style={{ flex: '0 0 280px', display: 'flex', flexDirection: 'column', gap: 11, paddingLeft: 36, borderLeft: '1px solid var(--border-subtle)' }}>
              {[
                { label: 'Contract',     value: '0x4e2a…91cF' },
                { label: 'Chain',        value: 'Arbitrum One' },
                { label: 'Royalty',      value: '2.5%' },
                { label: 'Listed Since', value: 'Nov 2025' },
              ].map(r => (
          ...
          <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
            ...
            {[...ASKS].reverse().map((a, i) => (
              <div key={i} style={{ position: 'relative', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', padding: '4px 14px', alignItems: 'center' }}>
                <span style={{ position: 'absolute', right: 0, top: 0, bottom: 0, background: 'rgba(246,70,93,0.08)', width: `${(a.size / maxSz) * 100}%` }} />
                <span style={{ fontSize: 12, fontFamily: 'var(--font-mono)', fontVariantNumeric: 'tabular-nums', color: 'var(--down-500)', position: 'relative' }}>{a.price.toFixed(2)}</span>
            ...
            <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', gap: 8, padding: '6px 14px', borderTop: '1px solid var(--border-subtle)', borderBottom: '1px solid var(--border-subtle)' }}>
              ...
              <span style={{ fontSize: 12, fontFamily: 'var(--font-mono)', fontWeight: 700, color: 'var(--text-secondary)' }}>12.00</span>
              <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>0.49%</span>
            </div>
            {/* Bids */}
            {BIDS.map((b, i) => (
              <div key={i} style={{ position: 'relative', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', padding: '4px 14px', alignItems: 'center' }}>
                <span style={{ position: 'absolute', right: 0, top: 0, bottom: 0, background: 'rgba(46,189,133,0.08)', width: `${(b.size / maxSz) * 100}%` }} />
                <span style={{ fontSize: 12, fontFamily: 'var(--font-mono)', fontVariantNumeric: 'tabular-nums', color: 'var(--up-500)', position: 'relative' }}>{b.price.toFixed(2)}</span>
        ...
        <div style={{ flex: '0 0 320px', background: 'var(--surface-1)', border: '1px solid var(--border-subtle)', borderRadius: 10, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
            {TOP_HOLDERS.map((h, i) => (
              <div key={i} style={{ display: 'grid', gridTemplateColumns: '1.6fr 0.7fr 0.7fr', alignItems: 'center', padding: '11px 16px', borderBottom: '1px solid var(--border-subtle)' }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 9, minWidth: 0 }}>
                  <span style={{ width: 26, height: 26, borderRadius: 6, background: h.swatch, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 900, color: '#fff', flexShrink: 0 }}>
                    {h.initials}
                  </span>
            ...
            <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 6 }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-tertiary)', letterSpacing: '0.06em' }}>DISTRIBUTION</span>
              <div style={{ height: 8, borderRadius: 4, overflow: 'hidden', display: 'flex' }}>
                <span style={{ width: '21%', background: 'var(--accent)' }} />
                <span style={{ width: '18%', background: '#6b3fa0' }} />
                <span style={{ width: '14%', background: '#4a2c70' }} />
                <span style={{ flex: 1, background: 'var(--surface-3)' }} />
              </div>
              <span style={{ fontSize: 11, color: 'var(--text-tertiary)' }}>Top 6 wallets hold 22.5% of supply</span>
            </div>
          </div>
        </div>
      </div>
